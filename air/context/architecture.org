...
- Single shared log for all events from all topics — sequential writes, optimal page cache
- Topics are a logical index, not separate files — no per-topic I/O overhead
- Events tagged with topic name in each record (2-byte topic_len header field)
- Mutex-serialized writes — fixes concurrency, simple and correct
- Segmented log files with in-memory position index for O(1) reads
