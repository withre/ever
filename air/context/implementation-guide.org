...
- Pass environ from =init.minimal.environ.block.slice.ptr= to child =execve=
...
- *StringArrayHashMap*: Still managed (stores allocator). =getOrPut= takes only key, not allocator.
- *Unmanaged allocator pattern*: Zig v0.16 deprecated Managed containers. Pass allocator per-call rather than storing in structs. Handler functions should accept allocator as an explicit parameter. This is the direction Zig is going — adopt it for long-term relevance.
