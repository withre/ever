...
There is no =--group= and no server-side offset commit, and this is a decision rather than a gap. Resumption is client-side: =ever on --name <n>= keeps a cursor file (=event-triggers.org= argues the case, including why putting consumer state in the store "makes the store a scheduler"), and =ever hook= carries a server-side cursor keyed to a registration when a durable, server-driven consumer is genuinely wanted. The =ack= opcode that once declared the group commit answered =ack_ok= and stored nothing for as long as it existed; it is gone (=v0.1/ack-opcode-removal.org=). Do not reintroduce a group flag without a named consumer and a reason those two cursors do not serve it.
...
error: connection refused at localhost:7890. Is the store running? Start with: ever store start
