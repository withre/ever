...
- 2026-08-08: =ack= / =ack_ok= (=0x06= / =0x86=) and the =AckRequest= body specified above are *retired*. The opcode shipped as =writeFrame(fd, .ack_ok, "{}")= and never decoded its body, so the consumer-group offset commit this spec declares was announced and never delivered. Removed rather than completed — the reasoning, including four prior refusals of server-side consumer groups in this project's own Non-Goals, is in [[./ack-opcode-removal.org]]. The bytes stay reserved. State left =complete=: the frame format, the error codes, and every other message type this document specifies are unchanged and still hold.
- 2026-08-08: Frame Reading above says "Validate msg_type is a known enum value". That validation was never written; =readFrame= called =@enumFromInt= on an exhaustive enum, which panics the process rather than validating anything. The requirement is now met, by a different mechanism than the wording implies — =MessageType= is non-exhaustive and the *dispatch* rejects unknown opcodes with =400=, so the decoder stays total and the client gets an error frame instead of a dropped connection. See [[./protocol-unknown-opcode.org]].
